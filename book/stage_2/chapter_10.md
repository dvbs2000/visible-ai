...
```mermaid
flowchart BT
    A["① 输入文本<br/>「今天天气真」"] --> B["② 分词 Tokenize<br/>今天 / 天气 / 真"]
    B --> C["③ 向量化 + 位置编码<br/>token→语义向量, 再补回语序"]
    C --> D["④ Transformer 块 ×N · 流水线的心脏<br/>自注意力(开圆桌) + 前馈网络(各自深加工)<br/>┄ 残差连接绕行 · 反复加工几十轮 ┄"]
    D --> E["⑤ Softmax 输出层<br/>给词表几万个词打分 → 压成总和 100% 概率"]
    E --> F["⑥ 预测下一个词<br/>好 58% · 不错 21% · 冷 9%"]
```
